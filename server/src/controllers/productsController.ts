...
class ProductsControllers {
  public async getProductsByArtistAndType(req: Request, res: Response) {
    const products = await pool.query('SELECT * FROM producto WHERE artista_id_artista = ? AND tipo_producto_id_tp = ?', [req.params.id_artist, req.params.id_type]);
    res.json(products);
  }

  public async getProductsByArtist(req: Request, res: Response) {
    const products = await pool.query('SELECT * FROM producto WHERE artista_id_artista = ?', [req.params.id]);
    res.json(products);
  }
  public async getProductsByType(req: Request, res: Response) {
    const products = await pool.query('SELECT * FROM producto WHERE tipo_producto_id_tp = ?', [req.params.id]);
    res.json(products);
  }
